...
use crate::{client::factory::GrpcClientFactory, response::ApiResult};
...
#[derive(Debug, Clone)]
pub struct AppState {
    pub grpc_factory: GrpcClientFactory,
...
impl AppState {
    pub async fn new(grpc_addr: &str) -> ApiResult<Self> {
        // 创建 grpc factory
        let grpc_factory = GrpcClientFactory::new(grpc_addr).await?;
